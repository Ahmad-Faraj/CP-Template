...
// DFS traversal to detect bridges and assign strong orientations.
...
// Driver code to parse edges, find bridges, and assign valid orientations.
signed main() {
    int n, m;
    cin >> n >> m;
    adj.resize(n);
    ...
    // finding a graph orientation so that the number of SCCs is minimal
    // > — left to the right (as in the input)
    // < — the opposite.
    cout << comp_cnt + bridge_cnt << endl << orient << endl;
}
