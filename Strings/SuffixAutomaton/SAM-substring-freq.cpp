...
struct SuffixAutomaton {
    struct State {
        ...
        State() : len(0), link(-1), sz(0) { fill(next, next + 26, -1); }
    };
    ...
    // Time Complexity: O(LOG) per query
    int get_substring_frequency(int l, int r) {
        int state_id = get_state(l, r);
        return st[state_id].sz;
    }

    // Time Complexity: O(|P|) where |P| is the length of the pattern
    int count_occurrences(const string &pattern) {
        int curr = 0;
        for (char c : pattern) {
            if (st[curr].next[c - 'a'] == -1) {
                return 0;
            }
            curr = st[curr].next[c - 'a'];
        }
        return st[curr].sz;
    }
};
...
signed main() {
    ...
    while (k--) {
        ...
        cout << sam.count_occurrences(pattern) << "\n";
    }
