...
struct UndirectedCycle {
    ...
    // Usage: Initialize Eulerian cycle structure for an undirected graph.
    UndirectedCycle(int _n, int _m) : n(_n), m(_m), adj(n), degree(n, 0) {}
    ...
    // Time Complexity: O((V + E) log E)
    // Space Complexity: O(V + E)
    // Usage: Hierholzer's algorithm to explore and build the Eulerian cycle.
    void dfs(int node) {
    ...
    // Time Complexity: O(V)
    // Space Complexity: O(1)
    // Usage: Validate degrees to ensure an undirected Eulerian cycle is possible.
    bool check() {
        for (int i = 0; i < n; ++i) {
    ...
    // Time Complexity: O(V + E)
    // Space Complexity: O(V + E)
    // Usage: Check if all edges are in a single connected component.
    bool isConnected() {
        int start = -1;
        for (int i = 0; i < n; ++i) {
            if (degree[i] > 0) {
    ...
    // Time Complexity: O((V + E) log E)
    // Space Complexity: O(V + E)
    // Usage: Attempt to find and output the undirected Eulerian cycle.
    bool get() {
