...
struct LiChaoTree {
    ...
    void add_line(ll m, ll c) { add_segment(m, c, L, R, L, R, root); }

    // y = mx + c restricted to segment [l_query, r_query]
    void add_segment(ll m, ll c, int l_query, int r_query) { add_segment(m, c, L, R, l_query, r_query, root); }
    ...
    ll query(ll x, int l, int r, node *cur) {
        ...
        int mid = l + (r - l) / 2;
        if (x <= mid)
            ans = min(ans, query(x, l, mid, cur->left)); // change min to max
        else
            ans = min(ans, query(x, mid + 1, r, cur->right)); // change min to max
        return ans;
    }

    ll query(ll x) { return query(x, L, R, root); }
};