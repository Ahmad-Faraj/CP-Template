// ============================================================================
//  VERIFICATION SUBMISSION - Dinic.cpp
//
//  Submit this file, unchanged, to:
//      CSES 1694 - "Download Speed"
//      https://cses.fi/problemset/task/1694
//
//  Statement read from that page: n computers, m directed connections with
//  capacities, report the maximum flow from 1 to n. solve() matches it exactly.
//
//  Checked locally before you spend a submission:
//    - the official sample gives 6, which is the official answer
//    - at the constraint ceiling (n = 500, m = 1000, c = 1e9) it finishes in
//      about 130 ms, so the time limit is not the risk here
//
//  Why this problem for this template: max flow is one of the least
//  brute-forceable things in the library. The local suite checks the flow value
//  against a brute-force minimum cut over every vertex subset, but only for
//  graphs of 6 or 7 nodes. This exercises the same code at 500 nodes with
//  capacities at 1e9, where the total flow passes 2^31 and the long long
//  accounting actually matters.
//
//  Everything between the two markers is
//  CP-Template/Graph/Flow_And_Matching/Dinic.cpp byte for byte; only main() was
//  added, because our templates carry solve() and no main() by contract.
// ============================================================================

// ---------------- BEGIN VERBATIM COPY OF Graph/Flow_And_Matching/Dinic.cpp -----
