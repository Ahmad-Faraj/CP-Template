...
template <typename T> struct Sector {
    ...
    Sector(Point<T> _c, Point<T> _start, Point<T> _end) : c(_c), start(_start), end(_end) { r = c.distance(start); }
    ...
    double area() const { return 0.5 * r * r * getAngle(); }
