...
void solve(int tc) {
    ...
    // cout<<nl;
    for (int i = 1; i <= n; i++) {
        V[i] = lower_bound(compress.begin(), compress.end(), V[i]) - compress.begin();
        // cout<<V[i]<<sp;
    }
    // cout<<nl;
    for (int i = 1, u, v; i < n && cin >> u >> v; i++) G[u].push_back(v), G[v].push_back(u);
    MoTree<int> mo(n, m, G, V);
    mo.getData();
    auto ans = mo.getAnswers();
